...
import { Box, Typography, Button, TextField, Card, CardContent, Snackbar } from '@mui/material';
...
function AddChoreLog() {
  ...
  const fetchFamilyMembers = async () => {
    ...
    try {
      const response = await axiosInstance.get(`/api/family/${familyId}/getall`);
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      setSnackbarMessage(`Chore "${selectedChore.name}" logged for ${selectedMember.firstName}`);
      setSnackbarOpen(true);
  ...
  return (
    <LocalizationProvider dateAdapter={AdapterDateFns} adapterLocale={sv}>
      <Box sx={{ mt: 4 }}>
        <Typography variant="h4" gutterBottom>Add Chore Log</Typography>
        
        <Typography variant="h6" gutterBottom>Select Family Member</Typography>
            ...
            <div key={member.id}>
              <Card 
                sx={{ 
                  m: 1, 
                  cursor: 'pointer',
                  bgcolor: selectedMember?.id === member.id ? 'primary.light' : 'background.paper'
                }}
        ...
        <Typography variant="h6" gutterBottom sx={{ mt: 4 }}>Select Chore</Typography>
        <Slider {...sliderSettings}>
          {chores.map((chore) => (
            <div key={chore.id}>
              <Card 
                sx={{ 
                  m: 1, 
                  cursor: 'pointer',
                  bgcolor: selectedChore?.id === chore.id ? 'primary.light' : 'background.paper'
                }}
